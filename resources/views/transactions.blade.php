...
<form method="GET" action="{{ route('transactions.index') }}" class="w-full sm:flex-1">
    ...
    <div class="hidden sm:flex flex-shrink-0 self-start">
      <img src="{{ asset('assets/picture/logo.png') }}" 
        ...
        class="w-16 sm:w-20 md:w-24 lg:w-28 h-auto object-contain"> 
    </div>
</header>
</div>
@endsection
...
<div x-data="transactionManager()">
    <div class="w-full max-w-screen-xl">
        ...
        class="w-full sm:w-auto flex items-center justify-center md:justify-start px-4 sm:px-6 py-3 sm:py-4 bg-white border-2 border-[#e1e5e9] rounded-xl font-semibold text-gray-700 hover:bg-[#0B3B9F] hover:border-[#0B3B9F] hover:text-white transition shadow text-sm sm:text-base">
        ...
        <div class="flex gap-5 flex-1 flex-wrap">
            <div class="flex-1 p-6 bg-white border border-[#e1e5e9] rounded-xl shadow hover:shadow-lg transition animate-slideInLeft">
                ...
                <p class="text-2xl md:text-[25px] font-bold text-[#1f2937] mt-2">
    ...
    <div class="bg-white rounded-xl shadow p-4 sm:p-6 animate-fadeIn overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4 sm:mb-5">
                ...
                <div class="flex gap-2 flex-wrap">
                    <button @click="editSelected()" :disabled="selectedTransactions.length !== 1"
                        class="flex-1 sm:flex-none flex items-center justify-center px-3 sm:px-4 py-2 bg-blue-500 text-white text-xs sm:text-sm rounded-lg hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition">
                    ...
                    <button @click="deleteSelected()" :disabled="selectedTransactions.length === 0"
                        class="flex-1 sm:flex-none flex items-center justify-center px-3 sm:px-4 py-2 bg-red-500 text-white text-xs sm:text-sm rounded-lg hover:bg-red-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="mb-4 flex items-center gap-2">
              <label for="filterMonth" class="text-xs sm:text-sm font-medium text-gray-600">Filter Bulan:</label>
              <input 
              type="month" 
              id="filterMonth"
                  ...
                  <table class="w-full text-center border border-[#e1e5e9] rounded-lg overflow-hidden text-sm min-w-[800px] whitespace-nowrap">
                    ...
                    @forelse ($transactions as $trx)
                        <tr class="border-b border-[#e1e5e9] hover:bg-[#f8f9fa]"
                            ...
                            <td class="p-2 sm:p-4 text-left">{{ \Carbon\Carbon::parse($trx->date)->format('d/m/Y') }}</td>
                            ...
                            <td class="p-2 sm:p-4 text-center">
                                @if ($trx->attachment)
                                    <a href="{{ asset('storage/' . $trx->attachment) }}" target="_blank"
                                        class="inline-flex items-center px-2 sm:px-3 py-1 bg-[#0B3B9F] text-white text-xs rounded-md hover:bg-blue-700 transition">
                                        Download
                                    </a>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="p-6 sm:p-8 text-center text-gray-500 text-sm">
                                Tidak ada transaksi
                            </td>
                        </tr>
                    @endforelse
              ...
              <div class="flex items-start gap-3 mb-3">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex justify-between items-start gap-2 mb-2">
                    ...
                    <span class="{{ $trx->type == 'income' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-[#F20E0F]' }} px-2 py-1 rounded text-xs font-medium whitespace-nowrap">
                      {{ $trx->type == 'income' ? 'Pemasukan' : 'Pengeluaran' }}
                    </span>
                  </div>
                  <div class="grid grid-cols-2 gap-2 text-xs mb-2">
                    <div>
                      ...
                      <p class="font-medium">{{ $trx->payment == 'cash' ? 'Cash' : ($trx->payment == 'transfer' ? 'Transfer' : 'Giro') }}</p>
                    </div>
                    <div>
                      <p class="text-gray-600">Jumlah</p>
                      <p class="font-semibold text-blue-600">{{$trx->amount ? 'Rp ' . number_format($trx->amount, 0, ',', '.') : 'Rp 0'}}</p>
                    </div>
                  </div>
                  ...
                  <div class="text-xs mb-2">
                    ...
                    <p class="font-medium">{{ $trx->no_factur }}</p>
                  </div>
                @endif

                @if ($trx->attachment)
                  <a href="{{ asset('storage/' . $trx->attachment) }}" target="_blank"
                    class="inline-flex items-center px-2 py-1 bg-[#0B3B9F] text-white text-xs rounded hover:bg-blue-700 transition">
                    Download
                  </a>
                @endif
                
                @if ($trx->description)
                  <p class="text-xs text-gray-600 mt-2">{{ $trx->description }}</p>
                  @endif
                </div>
              </div>
            </div>
