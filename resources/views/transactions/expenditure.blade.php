...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 mt-4 md:mt-6 mb-6 md:mb-8">
        <div
            class="bg-gradient-to-br from-[#F20E0F] to-[#ff3d2f] rounded-lg shadow-md p-4 md:p-6 text-white
        ...
        <div
         ...
         hover:shadow-lg hover:from-[#ff1f1f] hover:to-[#ff5a4f] transition-all">
            <div class="flex items-center justify-between">
                ...
                <i class="fas fa-arrow-down text-2xl md:text-4xl opacity-30"></i>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div class="bg-white rounded-lg shadow-md p-4 md:p-6">
            <h3 class="text-lg md:text-xl text-center font-semibold text-[#F20E0F] mb-4">Transaksi Tahun
                {{ $year }}
            </h3>
    ...
    <div class="mt-6 md:mt-10">
        <a href="{{ route('categories.expenditure.export', ['slug' => $category->slug, 'year' => $year]) }}"
            class="inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-3 bg-white border-2 border-gray-200 rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#F20E0F] hover:text-white hover:border-[#F20E0F] transition">
            <img src="{{ asset('assets/picture/download.png') }}" alt="download"
                class="w-4 sm:w-5 h-4 sm:h-5 filter invert-0 hover:invert transition">
    ...
    @push('scripts')
        ...
        <script>
            function filterByCategory(selectedSlug) {
                ...
                window.location.href = `{{ route('category.expenditure', ['slug' => ':slug']) }}`.replace(':slug',
                        selectedSlug) +
