...
@section('content')
    <div class="mb-4 md:mb-6">
        ...
        <div class="hidden md:flex justify-end items-center">
            <div class="flex items-center gap-2 flex-col sm:flex-row sm:gap-3">
                ...
                <select id="yearFilterDesktop" class="px-2 sm:px-3 py-2 border border-gray-300 rounded-md text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-[#0B3B9F]" 
                    onchange="filterByYear(this.value)">
                    @foreach($availableYears as $availableYear)
                        <option value="{{ $availableYear }}" {{ $availableYear == $year ? 'selected' : '' }}>
                            {{ $availableYear }}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div class="bg-white rounded-lg shadow-md p-4 md:p-6">
            <h3 class="text-lg md:text-xl text-center font-semibold text-[#0B3B9F] mb-4">Transaksi per Bulan Tahun {{ $year }}</h3>
    ...
    <div class="mt-6 md:mt-10">
        <a href="{{ route('categories.income.export', ['category_id' => $category->id, 'year' => $year]) }}"
            class="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-3 sm:px-4 py-2 sm:py-3 bg-white border-2 border-gray-200 rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#0B3B9F] hover:text-white hover:border-[#0B3B9F] transition">
...
<script>
    ...
    function filterByCategory(categoryId) {
        ...
        window.location.href = `{{ route('category.income') }}?category_id=${categoryId}&year=${year}`;
    }
</script>
@endpush
@endsection