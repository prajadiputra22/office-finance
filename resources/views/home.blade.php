...
@section('content')
    <section class="mb-8" aria-labelledby="saldo-title">
        ...
        <section class="lg:hidden" aria-labelledby="saldo-title-mobile">
            ...
            <div class="grid grid-cols-2 gap-4 mb-3 md:mb-5">
                <article
                    class="bg-white p-5 md:p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
    ...
    <div class="md:hidden space-y-3 mb-5">
        <div x-data="{ open: false }" x-cloak class="bg-white rounded-lg border border-gray-200 shadow-sm">
            <button @click="open=!open"
                class="w-full flex justify-between items-center px-4 py-3 font-semibold text-left hover:bg-gray-50 cursor-pointer transition rounded-lg">
                <div class="flex items-center">
    ...
    <section class="mb-6 md:mb-8" aria-labelledby="chart-title">
        ...
        <div class="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200" role="img"
    ...
    <section class="bg-white border border-gray-200 p-4 md:p-6 rounded-xl shadow-sm" aria-labelledby="transaksi-title">
        <h2 id="transaksi-title" class="text-lg md:text-xl font-semibold mb-4 md:mb-6 text-gray-800">Transaksi Terbaru
        </h2>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($recentTransactions as $transaction)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td
                                        class="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-900">
                ...
                @foreach ($recentTransactions as $transaction)
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <div class="flex items-center justify-between mb-2">
                            <span
                                class="text-xs text-gray-600">{{ \Carbon\Carbon::parse($transaction->date)->format('d/m/Y') }}</span>
                            <span
                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                {{ $transaction->type == 'income' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800' }}">
                                {{ $transaction->type == 'income' ? 'Pemasukan' : 'Pengeluaran' }}
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <p
                                class="text-sm font-bold {{ $transaction->type == 'income' ? 'text-blue-600' : 'text-red-600' }} whitespace-nowrap">
                                {{ $transaction->type == 'income' ? '+' : '-' }} Rp
                                {{ number_format($transaction->amount, 0, ',', '.') }}
                            </p>
                        </div>
