...
<div x-data="{ openAddCategory: false, filterDropdown: false }">
        ...
        <div class="bg-white rounded-lg w-full max-w-md shadow-xl" @click.outside="openAddCategory = false">
            ...
            <form id="addCategoryForm" class="p-4">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <!-- Fixed button hover color and enabled button functionality -->
                    <button type="submit" id="submitBtn"
                        class="px-4 py-2 bg-[#0B3B9F] text-white rounded-md hover:bg-blue-700 transition-colors font-medium cursor-pointer">
                        Simpan
                    </button>
                </div>
            </form>
    ...
    <script>
        document.getElementById('addCategoryForm').addEventListener('submit', async function(e) {
            ...
            console.log("[v0] Sending form data:", Object.fromEntries(formData));
            ...
            try {
                ...
                try {
                    ...
                    alert('Error: Server response is not JSON. Check console for details.');
                    return;
                }
                ...
                if (!response.ok) {
                    ...
                    console.log("[v0] Validation errors from server");
                    return;
                }

                const modal = document.querySelector('[x-data*="openAddCategory"]');
                if (modal && modal._x_dataStack && modal._x_dataStack[0]) {
                    modal._x_dataStack[0].openAddCategory = false;
                } else if (window.Alpine) {
                    console.log("[v0] Alpine store method fallback");
                }
                ...
                console.log("[v0] Category saved successfully, reloading page");
                location.reload();
            } catch (error) {
